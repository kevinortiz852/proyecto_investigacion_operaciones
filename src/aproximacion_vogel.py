...
def vogel(costos, oferta, demanda):
    ...
    while np.any(oferta) and np.any(demanda):
        ...
        for i in range(m):
            if oferta[i] == 0:
                penalizaciones_fila.append(-1)
                continue
            fila_costos = [costos[i, j] for j in range(n) if demanda[j] > 0 and not usados[i, j]]
            if len(fila_costos) >= 2:
                fila_costos.sort()
                penalizaciones_fila.append(fila_costos[1] - fila_costos[0])
            elif len(fila_costos) == 1:
                penalizaciones_fila.append(fila_costos[0])
            else:
                penalizaciones_fila.append(-1)
        for j in range(n):
            if demanda[j] == 0:
                penalizaciones_col.append(-1)
                continue
            col_costos = [costos[i, j] for i in range(m) if oferta[i] > 0 and not usados[i, j]]
            if len(col_costos) >= 2:
                col_costos.sort()
                penalizaciones_col.append(col_costos[1] - col_costos[0])
            elif len(col_costos) == 1:
                penalizaciones_col.append(col_costos[0])
            else:
                penalizaciones_col.append(-1)
        # Buscar mayor penalización
        max_penalizacion = max(max(penalizaciones_fila), max(penalizaciones_col))
        if max_penalizacion == -1:
            break
        if max(penalizaciones_fila) >= max(penalizaciones_col):
            i = penalizaciones_fila.index(max_penalizacion)
            # Buscar menor costo en la fila
            min_j = -1
            min_costo = float('inf')
            for j in range(n):
                if demanda[j] > 0 and not usados[i, j] and costos[i, j] < min_costo:
                    min_costo = costos[i, j]
                    min_j = j
            asignacion = min(oferta[i], demanda[min_j])
            x[i, min_j] = asignacion
            oferta[i] -= asignacion
            ...
            j = penalizaciones_col.index(max_penalizacion)
            # Buscar menor costo en la columna
            min_i = -1
            min_costo = float('inf')
            for i in range(m):
                if oferta[i] > 0 and not usados[i, j] and costos[i, j] < min_costo:
                    min_costo = costos[i, j]
                    min_i = i
            asignacion = min(oferta[min_i], demanda[j])
            x[min_i, j] = asignacion
...
def main_vogel(page: ft.Page):
    ...
    def resolver_click(e):
        try:
            ...
            resultado_text.value = f"Costo total (Vogel): {costo_total:.2f}"

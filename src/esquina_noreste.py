...
def main_esquina_noroeste(page: ft.Page):
    ...
    def resolver_click(e):
        try:
            ...
            x = esquina_noroeste(costos, oferta, demanda)
            ...
            tabla_calculo.controls.append(ft.Row([
                ft.Text("Cálculo de costo por celda", weight=ft.FontWeight.BOLD, color="#00bcd4", size=16)
            ]))
            suma = 0
            for i, planta in enumerate(row_names):
            ...
            resultado_text.value = f"Costo total (esquina noroeste): {costo_total:.2f}"
